...
export const GET: APIRoute = async ({ url, request }) => {
  ...
  try {
    ...
    // If no ID provided, return a default image immediately
    if (!id) {
      console.log('No ID provided, returning default image');
      return createDefaultImage();
    }

    // Validate ID format to prevent potential issues
...
// Create a sentiment analysis image with better error handling
function createSentimentImage(data: {
  ...
  try {
    ...
    const svg = `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="200" y="275" width="${Math.max(5, (positive / 100) * 250)}" height="20" rx="10" fill="#059669"/>
      ...
      <rect x="600" y="275" width="${Math.max(5, (neutral / 100) * 250)}" height="20" rx="10" fill="#6b7280"/>
      ...
      <rect x="1000" y="275" width="${Math.max(2, (negative / 100) * 120)}" height="20" rx="10" fill="#dc2626"/>
    ...
    console.error('Error creating sentiment image:', error);
    return createErrorImage('Image Generation Error');
  }
}

// Create error image with better error handling
function createErrorImage(message: string) {
  ...
  const svg = `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="600" y="280" text-anchor="middle" fill="#dc2626" font-family="Arial, sans-serif" font-size="48" font-weight="bold">⚠️ ${escapeXml(message)}</text>
  ...
  return new Response(svg, {
    status: 200, // Return 200 instead of error status for OG images
    headers: {
      'Content-Type': 'image/svg+xml',
